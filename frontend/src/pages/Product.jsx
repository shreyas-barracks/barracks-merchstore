...
const Product = () => {
    ...
    const uploadImage = async () => {
        ...
        formData.append('upload_preset', 'barracks-merch');
        formData.append('cloud_name', 'dcbwleh5n');

        try {
            setIsUploading(true); // Start uploading
            const response = await fetch('https://api.cloudinary.com/v1_1/dcbwleh5n/image/upload', {
                method: 'POST',
                body: formData
            });
                    ...
                    <>
                        {
                            product.image2 ? <Carousel className='w-6/6' showThumbs={false} infiniteLoop={true} autoPlay={true} showStatus={false} showArrows={true}>
                                ...
                                <Carousel className='w-full md:w-3/4' showThumbs={false} infiniteLoop={true} autoPlay={true} showStatus={false} showArrows={true}>
                                        ...
                                        <div className='flex flex-col'>
                                            <label htmlFor='size' className='text-l'>Size:
                                                {product.size_chart_image && (
                                                    <a href={api_url + product.size_chart_image} target='_blank' rel='noreferrer' className='ml-2 text-blue-500 text-xs'>
                                                        Size Chart
                                                    </a>
                                                )}
                                            </label>
